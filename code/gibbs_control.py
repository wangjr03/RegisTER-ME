...
def count_probability(round_decial,max_density,length,counts,ps,pn):
   density=round(float(counts+1)/length*100,round_decial)
   if (density>max_density):
      density=max_density
   probability=ps[density]
...
###get all_sites_file 
os.system('cat '+ within_peak_file + '>' + all_sites_file)
os.system('cat '+ outside_peak_file + '>>' + all_sites_file)
   ...
   if (iter==1):
      for every_tag in ambiguous:
         ...
         for every_site in ambiguous[every_tag]:
            ...
            possibility.append(count_probability(round_decimal,max_density,length_of_site,site[chrom][(start,end)],uniq_ps,uniq_pn))
   ...
   if (iter!=1):
      ...
      for every_tag in ambiguous:
         ...
         for every_site in ambiguous[every_tag]:
            ...
            length_of_site=end-start+1
            possibility.append(count_probability(round_decimal,max_density,length_of_site,site[chrom][(start,end)],ps,pn))
