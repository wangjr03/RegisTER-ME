...
def modify_possibility(possibility):
   if sum(possibility)==0:
      return [x+1 for x in possibility]
   elif sum(possibility) > sys.float_info.max: 
      for i in range(len(possibility)):
         if possibility[i] > sys.float_info.max: possibility[i] = 1
         else: possibility[i] = possibility[i]/1e+308
   return possibility
...
def modify_likelihood(ps, pn):
   density = list(sorted(ps.keys()))
   for i in range(1,len(density)):
      if pn[density[i]]==0:
         ps[density[i]], pn[density[i]] = ps[density[i-1]], pn[density[i-1]]
   return ps, pn
...
uniq_ps, uniq_pn = modify_likelihood(uniq_ps, uniq_pn)
...
while (iter<=max_iteration    and   (threshold>threshold_we_set   or  fraction_change>fraction_change_we_set) ):
   ...
   if (iter==1):
      for every_tag in ambiguous:
         ...
         possibility=[]
         for chrom,start,end in ambiguous[every_tag]:
         ...
         chrom,start,end=ambiguous[every_tag][index]
